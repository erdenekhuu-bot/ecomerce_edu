...
class ProductController extends Controller
{
    ...
    {
        $tags = Product::find($product_id)->tags; // Example string
        ...
        // })->where('id', '!=', $product_id)->orwhere(function ($query) use ($tagsArray) {
        //     foreach ($tagsArray as $tag) {
        //         $query->orWhere('tags', 'LIKE', "%$tag%");
        //     }
        // })->where('id', '!=', $product_id)->limit(10)->get();
        
        if (isset($tags) && $tags != "" && $tags != null) {
            $products = filter_products(Product::query())->where(function ($query) use ($tagsArray, $tags) {
                if (isset($tags) && $tags != "" && $tags != null) {
                    foreach ($tagsArray as $tag) {
                        $query->orWhere('tags', 'LIKE', "%$tag%");
                    }
    ...
    public function search(Request $request)
    {
       ...
       if ($selected_attribute_values) {  
        ...
        return response()->json([
            ...
            'brandMetaThumbnail'=> $brand ? Upload::find($brand->logo)->file_name : null,
            'brandMetaAdditional'=>$brand ? Upload::find($brand->additional_description)->file_name ?? null : "",
            'products' => $collection,
