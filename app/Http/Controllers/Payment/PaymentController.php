...
// dd(session()->all());
        ...
        if ($gateway == 'paypal') {
            ...
            $payment_data['order_no'] = "WLT".date('Ymd') . "-" . rand(1000, 9999);
            $qpay = new QpayController;
            $qpay_auth_response = $qpay->authenticate();

            $send['bill_no'] = $payment_data['order_no'];
            $send['amount'] = $payment_data['amount'];
            $qpay_create_response = $qpay->createWallet($send);
            $qpay_create = json_decode($qpay_create_response);
            $payment_details = $qpay_create->qr_image;
