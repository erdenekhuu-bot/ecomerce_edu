(function ($) {
  ...
  $(document).ready(function () {
    ...
    $.each(all_converter, function (e) {
      ...
      $.each(tree, function (i, e) {
        ...
        var numHyphen_next =
          numHyphen_nextMatch != null ? numHyphen_nextMatch[0].length : 0;
  ...
  $(this).on("nodeExpanded nodeCollapsed", function () {
    ...
    $.each(this_ul, function (e) {
      ...
      if ($(this).is(":visible")) {
        //console.log("visible"
        $(".hummingbird-base").hummingbird("enableNode", {
          sel: "id",
          vals: [parent_node_id],
        });
      } else {
        //console.log(" not visible")
        $(".hummingbird-base").hummingbird("disableNode", {
          sel: "id",
          vals: [parent_node_id],
        });
      }
  ...
  $.fn.hummingbird.checkAll = function (tree) {
    ...
    var unchecked_end_nodes = tree.find(
      "input:checkbox.hummingbird-end-node:not(:disabled)"
    );
    //check those
  ...
  $.fn.hummingbird.uncheckAll = function (tree) {
    ...
    var checked_end_nodes = tree.find(
      "input:checkbox:checked.hummingbird-end-node:not(:disabled)"
    );
    //uncheck those
  ...
  $.fn.hummingbird.checkNode = function (tree, sel, vals) {
    ...
    if (sel == "text") {
      ...
      $.each(vals, function (i, e) {
        ...
        var the_node = tree
          .find("input:checkbox:not(:disabled)[" + sel + '="' + e + '"]')
          .prop("indeterminate", false)
          .prop("checked", true);
        //no return needed
        $.fn.hummingbird.checkboxClicked(the_node);
  ...
  $.fn.hummingbird.disableToggle = function (tree, sel, vals) {
    $.each(vals, function (i, e) {
      if (sel == "text") {
        ...
        var that_nodes = tree
          .find("input:checkbox:not(:checked)[" + sel + '="' + e + '"]')
          .parent("label");
        //console.log("else")
        //console.log(that_nodes)
      }
  ...
  $.fn.hummingbird.addNode = function (
    ...
    $.each(anchor_vals, function (i, e) {
      ...
      if (anchor_sel[i] == "text") {
        ...
        var that_node = tree
          .find("input:checkbox[" + anchor_sel[i] + '="' + e + '"]')
          .parent("label")
          .parent("li");
      }
      //
    ...
    $.each(vals, function (i, e) {
      if (sel == "text") {
        ...
        var this_checkbox = tree.find(
          "input:checkbox:not(:disabled)[" + sel + '="' + e + '"]'
        );
      }
  ...
  //-------------------disableParentNodeOnCollapse---------------//
  $.fn.hummingbird.disableParentNodeOnCollapse = function (
    tree,
    sel,
    vals,
    state
    ...
    $.each(vals, function (i, e) {
      ...
      if (state) {
        this_checkbox
          .parent("label")
          .next("ul")
          .addClass("disableParentNodeOnCollapse");
      } else {
        this_checkbox
          .parent("label")
          .next("ul")
          .removeClass("disableParentNodeOnCollapse");
      }
  ...
  $.fn.hummingbird.getUnchecked = function (
    ...
    if (onlyEndNodes == true) {
      tree
        .find("input:checkbox.hummingbird-end-node:not(:checked)")
        .each(function () {
          list.text.push($(this).parent("label").parent("li").text());
          list.id.push($(this).attr("id"));
          list.dataid.push($(this).attr("data-id"));
      ...
      if (onlyParents == true) {
        tree
          .find("input:checkbox:not(:checked):not(.hummingbird-end-node)")
          .each(function () {
            list.text.push($(this).parent("label").parent("li").text());
            list.id.push($(this).attr("id"));
            list.dataid.push($(this).attr("data-id"));
  ...
  $.fn.hummingbird.triState = function (tree, restrict) {
    ...
    $.each(the_uls, function (i, e) {
      ...
      //count checked
      var checked_num_the_checkboxes = $(this)
        .children("li")
        .children("label")
      ...
      // console.log(checked_num_the_checkboxes)
      //count disabled
      var disabled_num_the_checkboxes = $(this)
        .children("li")
        .children("label")
      ...
      $(this)
        ...
        .map(function () {
          indeterminate_num_the_checkboxes =
            indeterminate_num_the_checkboxes + $(this).prop("indeterminate");
        });
      //
      ...
      if (
        checked_num_the_checkboxes < num_the_checkboxes &&
        checked_num_the_checkboxes > 0
      ) {
        //console.log("case 2")
        var this_state = $(this)
          .parent("li")
