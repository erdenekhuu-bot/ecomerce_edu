...
<div class="col-lg-8 mx-auto">
    <div class="card">
        ...
        <form action="{{ route('delivery-boy.update', $delivery_boy->id) }}" method="POST">
            @csrf
                    ...
                    <div class="col-sm-10">
                        <select class="form-control aiz-selectpicker" name="country_id" id="country_id" required>
                            ...
                            @foreach ($countries as $country)
                                <option value="{{ $country->id }}" @if($address->country == $country->name) selected @endif>
                                    {{ $country->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-10">
                        <select class="form-control mb-3 aiz-selectpicker" name="state_id" id="edit_state"  data-live-search="true" required>
                            @foreach ($states as $key => $state)
                                <option value="{{ $state->id }}" @if($address->state == $state->name) selected @endif>
                                    {{ $state->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-10">
                        <select class="form-control mb-3 aiz-selectpicker" data-live-search="true" name="city_id" required>
                            @foreach ($cities as $key => $city)
                                <option value="{{ $city->id }}" @if($address->city == $city->name) selected @endif>
                                    {{ $city->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="false">
                            ...
                            <input type="hidden" name="avatar_original" value="{{ $delivery_boy->avatar }}" class="selected-files">
                        </div>
