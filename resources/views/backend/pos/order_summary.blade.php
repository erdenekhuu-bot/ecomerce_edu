	                    ...
	                    <div class="col">
	                    	<div class="d-flex">
	                    		@if($product_variation->image == null)
	                    			<img src="{{ uploaded_asset($product_variation->product->thumbnail_img) }}" class="img-fit size-60px">
	                    		@else
	                    			<img src="{{ uploaded_asset($product_variation->product->image) }}" class="img-fit size-60px">
	                    		@endif
	                    		<span class="flex-grow-1 ml-3 mr-0">
			                        <div class="text-truncate-2">{{ $product_variation->product->name }}</div>
			...
			<div class="card mb-4">
				...
				<div class="card-body">
					@if(Session::has('pos.shipping_info') && Session::get('pos.shipping_info')['name'] != null)
						...
						<div class="d-flex justify-content-between  mb-2">
							<span class="">{{translate('Wallet Amount')}}:</span>
							<span class="fw-600">{{ Session::get('pos.shipping_info')['wallet_amount'] }}</span>
						</div>
					@else
