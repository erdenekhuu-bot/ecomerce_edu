                        ...
                        <tr>
                            ...
                            <td>
                                @foreach (json_decode($zone->cities ?? '[]') as $city_id)
                                    @php $city = App\Models\State::find($city_id); @endphp
                                    @if ($city)
                                        <span
                                            class="badge badge-inline badge-md bg-soft-dark mb-1">{{ $city->name }}</span>
                                    @endif
                                @endforeach
                            </td>
                            <td>
                            ...
                            <td class="text-right">
                                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                    href="{{ route('zones.edit', $zone->id) }}" title="{{ translate('Edit') }}">
                                    <i class="las la-edit"></i>
                                </a>
                                <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                                    data-href="{{ route('zones.destroy', $zone->id) }}" title="{{ translate('Delete') }}">
